...
;function.c,97 :: 		PORTI4_bit=DIPSW;
	...
	BCF         _PORTI+0, 4 
	GOTO        L__UpdateExternalPorts104
L__UpdateExternalPorts103:
	BSF         _PORTI+0, 4 
L__UpdateExternalPorts104:
;function.c,98 :: 		break;
	GOTO        L_UpdateExternalPorts26
...
;function.c,100 :: 		PORTI5_bit=DIPSW;
	BTFSC       RA5_bit+0, BitPos(RA5_bit+0) 
	GOTO        L__UpdateExternalPorts105
	BCF         _PORTI+0, 5 
	GOTO        L__UpdateExternalPorts106
L__UpdateExternalPorts105:
	BSF         _PORTI+0, 5 
L__UpdateExternalPorts106:
...
;function.c,103 :: 		PORTI6_bit=DIPSW;
	BTFSC       RA5_bit+0, BitPos(RA5_bit+0) 
	GOTO        L__UpdateExternalPorts107
	BCF         _PORTI+0, 6 
	GOTO        L__UpdateExternalPorts108
L__UpdateExternalPorts107:
	BSF         _PORTI+0, 6 
L__UpdateExternalPorts108:
...
;function.c,106 :: 		PORTI7_bit=DIPSW;
	BTFSC       RA5_bit+0, BitPos(RA5_bit+0) 
	GOTO        L__UpdateExternalPorts109
	BCF         _PORTI+0, 7 
	GOTO        L__UpdateExternalPorts110
L__UpdateExternalPorts109:
	BSF         _PORTI+0, 7 
L__UpdateExternalPorts110:
...
;interrupt.c,7 :: 		}
L_Interrupt73:
;interrupt.c,10 :: 		if(RCIE_bit && RCIF_bit){   // Eger UART veri alma kesmesi aktifse ve kesme bayragi set edilmis ise
	BTFSS       RCIE_bit+0, BitPos(RCIE_bit+0) 
	GOTO        L_Interrupt76
	BTFSS       RCIF_bit+0, BitPos(RCIF_bit+0) 
	GOTO        L_Interrupt76
L__Interrupt91:
;interrupt.c,11 :: 		RCIF_bit=0;               // Kesme bayragini temizle
	BCF         RCIF_bit+0, BitPos(RCIF_bit+0) 
;interrupt.c,12 :: 		if(UART1_Data_Ready()){   // Eger UART verisi okumaya hazir ise
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSC       STATUS+0, 2 
	GOTO        L_Interrupt77
;interrupt.c,13 :: 		receivedText[receivedBytes]=UART1_Read();    // Gelen veriyi oku ve receivedText dizisine yaz
	MOVLW       _receivedText+0
	MOVWF       FLOC__Interrupt+0 
	MOVLW       hi_addr(_receivedText+0)
	MOVWF       FLOC__Interrupt+1 
	MOVF        _receivedBytes+0, 0 
	...
	MOVF        R0, 0 
	MOVWF       POSTINC1+0 
;interrupt.c,14 :: 		receivedBytes++;                             // Gelen veri sayacini 1 arttir
	INCF        _receivedBytes+0, 1 
;interrupt.c,15 :: 		}
L_Interrupt77:
;interrupt.c,16 :: 		}
L_Interrupt76:
...
;ds18b20.c,30 :: 		if(tempData>0xF800){
	...
	SUBLW       248
	BTFSS       STATUS+0, 2 
	GOTO        L__ReadDS18B20119
	MOVF        R2, 0 
	SUBLW       0
L__ReadDS18B20119:
	BTFSC       STATUS+0, 0 
	GOTO        L_ReadDS18B2079
...
;main.c,15 :: 		InitMCU();
	CALL        _InitMCU+0, 0
;main.c,18 :: 		counter=EEPROM_Read(0x00);       // EEPROM 0x00 adresinden ilk byte degerini oku ve sayaca yaz
	...
	MOVLW       0
	MOVWF       _counter+1 
;main.c,19 :: 		counter<<=8;                     // Sayaci 8 bit sola kaydir
	MOVF        _counter+0, 0 
	MOVWF       _counter+1 
	CLRF        _counter+0 
;main.c,20 :: 		counter+=EEPROM_Read(0x01);      // EEPROM 0x01 adresinden ikinci byte degerini oku ve sayaca ekle
	MOVLW       1
	...
	MOVLW       0
	ADDWFC      _counter+1, 1 
;main.c,26 :: 		Lcd_Out(1,1,"13.UART Haberles");         // Ekranda 1.satir 1.sütundan itibaren yaz
	MOVLW       1
	...
	CALL        _Lcd_Out+0, 0
;main.c,27 :: 		Lcd_Out(2,1,"me Uygulamasi   ");         // Ekranda 2.satir 1.sütundan itibaren yaz
	MOVLW       2
	MOVWF       FARG_Lcd_Out_row+0 
	MOVLW       1
	MOVWF       FARG_Lcd_Out_column+0 
	...
	MOVWF       FARG_Lcd_Out_text+1 
	CALL        _Lcd_Out+0, 0
;main.c,29 :: 		wait(2000);                              // 2 saniye bekleme
	MOVLW       208
	MOVWF       FARG_wait_time+0 
	MOVLW       7
	MOVWF       FARG_wait_time+1 
	CALL        _wait+0, 0
;main.c,30 :: 		Lcd_Cmd(_LCD_CLEAR);
	MOVLW       1
	MOVWF       FARG_Lcd_Cmd_out_char+0 
	CALL        _Lcd_Cmd+0, 0
;main.c,32 :: 		UART1_Init(9600);                        // UART 9600 baud hizinda hazirlaniyor
	BSF         BAUDCON+0, 3, 0
	MOVLW       4
	MOVWF       SPBRGH+0 
	MOVLW       225
	...
	BSF         TXSTA+0, 2, 0
	CALL        _UART1_Init+0, 0
;main.c,33 :: 		RCIE_bit=1;                              // UART veri alma kesmesi aktif
	BSF         RCIE_bit+0, BitPos(RCIE_bit+0) 
;main.c,36 :: 		for(i=0;i<32;i++)
	CLRF        _i+0 
L_main80:
	MOVLW       32
	SUBWF       _i+0, 0 
...
;main.c,39 :: 		while(1){
L_main83:
;main.c,40 :: 		receivedBytes=0;                       // Alinan veri sayacimizi sifirliyoruz
	CLRF        _receivedBytes+0 
;main.c,41 :: 		UART1_Write_Text("Merhaba Dunya!  ");  // 16 karakter veri gonderiyoruz
	MOVLW       ?lstr3_main+0
	MOVWF       FARG_UART1_Write_Text_uart_text+0 
	MOVLW       hi_addr(?lstr3_main+0)
	MOVWF       FARG_UART1_Write_Text_uart_text+1 
	CALL        _UART1_Write_Text+0, 0
;main.c,42 :: 		UART1_Write_Text("Sayac:    ");        // 16 karakter veri gonderiyoruz
	MOVLW       ?lstr4_main+0
	MOVWF       FARG_UART1_Write_Text_uart_text+0 
	MOVLW       hi_addr(?lstr4_main+0)
	MOVWF       FARG_UART1_Write_Text_uart_text+1 
	CALL        _UART1_Write_Text+0, 0
;main.c,44 :: 		counter++;                             // Sayaci bir arttirma
	INFSNZ      _counter+0, 1 
	INCF        _counter+1, 1 
;main.c,45 :: 		IntToStrWithZeros(counter,txt);        // Sayaci metin formatina cevirme
	MOVF        _counter+0, 0 
...
;main.c,46 :: 		UART1_Write_Text(txt);                 // 6 karakter veri gonderiyoruz
	MOVLW       _txt+0
	MOVWF       FARG_UART1_Write_Text_uart_text+0 
	MOVLW       hi_addr(_txt+0)
	MOVWF       FARG_UART1_Write_Text_uart_text+1 
	CALL        _UART1_Write_Text+0, 0
;main.c,51 :: 		for(i=0;i<16;i++){
	CLRF        _i+0 
L_main85:
	MOVLW       16
	SUBWF       _i+0, 0 
	BTFSC       STATUS+0, 0 
	GOTO        L_main86
;main.c,52 :: 		Lcd_Chr(1,i+1,receivedText[i]);      // i=0 icin LCD_OUT(1,1,... olmalidir
	MOVLW       1
	...
	CALL        _Lcd_Chr+0, 0
;main.c,53 :: 		Lcd_Chr(2,i+1,receivedText[i+16]);   // i+1 ler bu sebeple eklenmistir
	MOVLW       2
	MOVWF       FARG_Lcd_Chr_row+0 
	MOVF        _i+0, 0 
	ADDLW       1
...
;main.c,58 :: 		receivedText[i]=' ';
	GOTO        L_main88
L_main89:
;main.c,60 :: 		wait(1000);                              // 1 saniye bekleme
	MOVLW       232
	MOVWF       FARG_wait_time+0 
	MOVLW       3
	MOVWF       FARG_wait_time+1 
	CALL        _wait+0, 0
;main.c,63 :: 		EEPROM_Write(0x00,counter>>8);   // EEPROM 0x00 adresine counter yüksek byte degerini yaz
...
;main.c,64 :: 		EEPROM_Write(0x01,counter);      // EEPROM 0x01 adresine counter dusuk byte degerini yaz
	MOVLW       1
