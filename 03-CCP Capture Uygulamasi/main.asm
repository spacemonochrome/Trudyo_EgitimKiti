...
;initialize.c,40 :: 		Lcd_Cmd(_LCD_CLEAR);
	...
	MOVWF       FARG_Lcd_Cmd_out_char+0 
	CALL        _Lcd_Cmd+0, 0
;initialize.c,43 :: 		TRISC1_bit=1;          // PWM çikisini Capture modu için giris olarak guncelliyoruz
	BSF         TRISC1_bit+0, BitPos(TRISC1_bit+0) 
;initialize.c,47 :: 		CCP2CON=0b00000111;    // [-][-][DC1B1][DC1B0][CCP1M3][CCP1M2][CCP1M1][CCP1M0]   -> Capture Mode Every 16th Rising Edge
	MOVLW       7
	MOVWF       CCP2CON+0 
;initialize.c,48 :: 		CCP2IE_bit=1;          // CCP2 kesmesi aktif ediliyor
...
;interrupt.c,7 :: 		}
L_Interrupt73:
;interrupt.c,9 :: 		if(CCP2IE_bit && CCP2IF_bit){    // Eger CCP2 kesmesi aktifse ve kesme bayragi set edilmis ise
	BTFSS       CCP2IE_bit+0, BitPos(CCP2IE_bit+0) 
	GOTO        L_Interrupt76
	BTFSS       CCP2IF_bit+0, BitPos(CCP2IF_bit+0) 
	GOTO        L_Interrupt76
L__Interrupt86:
;interrupt.c,10 :: 		counter++;                     // Sayaci bir arttir
	INFSNZ      _counter+0, 1 
	INCF        _counter+1, 1 
;interrupt.c,11 :: 		CCP2IF_bit=0;                  // Kesme bayragini sifirla
...
;ds18b20.c,30 :: 		if(tempData>0xF800){
	...
	SUBLW       248
	BTFSS       STATUS+0, 2 
	GOTO        L__ReadDS18B20114
	MOVF        R2, 0 
	SUBLW       0
L__ReadDS18B20114:
	BTFSC       STATUS+0, 0 
	GOTO        L_ReadDS18B2078
...
;main.c,14 :: 		InitMCU();
	CALL        _InitMCU+0, 0
;main.c,16 :: 		Lcd_Out(1,1,"3.CCP Capture   ");    // Ekranda 1.satir 1.sütundan itibaren yaz
	MOVLW       1
	...
	CALL        _Lcd_Out+0, 0
;main.c,17 :: 		Lcd_Out(2,1,"Capture Cnt:   0");    // Ekranda 2.satir 1.sütundan itibaren yaz
	MOVLW       2
	MOVWF       FARG_Lcd_Out_row+0 
	MOVLW       1
	MOVWF       FARG_Lcd_Out_column+0 
...
;main.c,19 :: 		while(1){
L_main79:
;main.c,20 :: 		counter =0;             // Sayac sifirlaniyor;
	CLRF        _counter+0 
	CLRF        _counter+1 
;main.c,23 :: 		for(i=0;i<1600;i++){
	CLRF        _i+0 
	CLRF        _i+1 
L_main81:
	MOVLW       6
...
L__main116:
	BTFSC       STATUS+0, 0 
	GOTO        L_main82
;main.c,24 :: 		IN4=1;                // IN4 cikisini aktif et
	BSF         LATB4_bit+0, BitPos(LATB4_bit+0) 
;main.c,25 :: 		delay_us(1);          // 1ms bekle
	MOVLW       3
	MOVWF       R13, 0
L_main84:
	DECFSZ      R13, 1, 1
	...
	NOP
	NOP
;main.c,26 :: 		IN4=0;                // IN4 cikisini pasif et
	BCF         LATB4_bit+0, BitPos(LATB4_bit+0) 
;main.c,27 :: 		delay_us(1);          // 1ms bekle
	MOVLW       3
	MOVWF       R13, 0
L_main85:
	DECFSZ      R13, 1, 1
...
;main.c,28 :: 		}
	GOTO        L_main81
L_main82:
;main.c,30 :: 		IntToStr(counter,txt);
	MOVF        _counter+0, 0 
