...
;initialize.c,11 :: 		TRISA=0b00101010;// [][][DIPSW][LED][TRIMPOT2][BUZZER][TRIMPOT1][RELAY]
	MOVLW       42
	MOVWF       TRISA+0 
;initialize.c,12 :: 		TRISB=0b11100100;// [BUTTON_DOWN][BUTTON_UP][BUTTON_OK][IN4][DS][BUTTON_MENU][][]
	MOVLW       228
	MOVWF       TRISB+0 
...
;interrupt.c,7 :: 		}
L_Interrupt73:
;interrupt.c,9 :: 		if(IOCIF_bit){
	BTFSS       IOCIF_bit+0, BitPos(IOCIF_bit+0) 
	GOTO        L_Interrupt74
;interrupt.c,14 :: 		if(!BUTTON_OK)                 // Eger OK butonuna basilmis ise
	BTFSC       RB5_bit+0, BitPos(RB5_bit+0) 
	GOTO        L_Interrupt75
;interrupt.c,15 :: 		number=0;                    // sayaci sifirla
	CLRF        _number+0 
L_Interrupt75:
;interrupt.c,17 :: 		if(!BUTTON_UP && number<99)    // Eger UP butonuna basilmis ise ve sayacimiz 99'dan kucuk ise
	...
	GOTO        L_Interrupt78
L__Interrupt90:
;interrupt.c,18 :: 		number++;                    // sayaci bir arttir
	INCF        _number+0, 1 
L_Interrupt78:
;interrupt.c,20 :: 		if(!BUTTON_DOWN && number>0)   // Eger DOWN butonuna basilmis ise ve sayacimiz 0'dan buyuk ise
	...
	GOTO        L_Interrupt81
L__Interrupt89:
;interrupt.c,21 :: 		number--;                    // sayaci bir azalt
	DECF        _number+0, 1 
L_Interrupt81:
;interrupt.c,23 :: 		PrintInt(number);              // Sayaci ekrana yaz
	MOVF        _number+0, 0 
	MOVWF       FARG_PrintInt_number+0 
	CALL        _PrintInt+0, 0
;interrupt.c,28 :: 		while(!BUTTON_OK || !BUTTON_UP || !BUTTON_DOWN);  // Butonlar birakilana kadar bekle
L_Interrupt82:
...
L__Interrupt88:
	GOTO        L_Interrupt82
L_Interrupt83:
;interrupt.c,30 :: 		IOCIF_bit=0;       // degisiklik kesme bayragini sifirliyoruz.
...
;main.c,11 :: 		InitMCU();
	CALL        _InitMCU+0, 0
;main.c,12 :: 		IOCIE_bit=1;    // PORTB degisiklik kesmesini aktif ediyoruz.
	BSF         IOCIE_bit+0, BitPos(IOCIE_bit+0) 
;main.c,13 :: 		IOCB4_bit=0;    // B4 pini degisiklik kesmesi kapali
	BCF         IOCB4_bit+0, BitPos(IOCB4_bit+0) 
;main.c,14 :: 		IOCB5_bit=1;    // B4 pini degisiklik kesmesi acik
	BSF         IOCB5_bit+0, BitPos(IOCB5_bit+0) 
;main.c,15 :: 		IOCB6_bit=1;    // B4 pini degisiklik kesmesi acik
	BSF         IOCB6_bit+0, BitPos(IOCB6_bit+0) 
;main.c,16 :: 		IOCB7_bit=1;    // B4 pini degisiklik kesmesi acik
	BSF         IOCB7_bit+0, BitPos(IOCB7_bit+0) 
;main.c,20 :: 		Lcd_Out(1,1,"10.PORTB Dgsklik");         // Ekranda 1.satir 1.sütundan itibaren yaz
	MOVLW       1
	...
	CALL        _Lcd_Out+0, 0
;main.c,21 :: 		Lcd_Out(2,1,"Kesm. ile sayici");         // Ekranda 2.satir 1.sütundan itibaren yaz
	MOVLW       2
	MOVWF       FARG_Lcd_Out_row+0 
	MOVLW       1
	MOVWF       FARG_Lcd_Out_column+0 
